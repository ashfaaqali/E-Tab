...
sealed class DrawAction {
    data class Stroke(val path: Path, var points: List<Pair<Float, Float>>, val color: Int, val strokeWidth: Float, val isEraser: Boolean = false, var page: Int = 1) : DrawAction()
    data class Text(val text: String, var x: Float, var y: Float, val color: Int, var textSize: Float, var page: Int = 1) : DrawAction()
